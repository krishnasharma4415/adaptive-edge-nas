...
# We load datasets once for all baseline models instead of recreating loaders per model
# Wait, the dataset uses persistent workers so it might be better to init loaders inside the loop or outside
# We will initialize inside the loop to match original exactly
...
for name in MODELS_TO_TRAIN:
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        model.eval()
        total_loss, correct1, correct5, v_total = 0., 0, 0, 0
        with torch.no_grad():
            for batch_idx, (imgs, labels) in enumerate(val_loader):
                ...
                total_loss += v_loss.item() * imgs.size(0)
                v_total      += imgs.size(0)
        ...
        val_loss = total_loss / v_total
        val_acc1 = correct1 / v_total
        val_acc5 = correct5 / v_total
